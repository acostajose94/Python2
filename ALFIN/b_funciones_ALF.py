...
def aplicar_reemplazo(sheet, dato_a_reemplazar, dato_que_reemplazara, *multiples_columnas_a_remplazar):
    for columna in multiples_columnas_a_remplazar:
        for fila in sheet.iter_rows(values_only=True):
            for indice, valor in enumerate(fila):
                if indice == columna and valor == dato_a_reemplazar:
                    fila[indice] = dato_que_reemplazara  
...
def rellenar(sheet, dato_a_poner, *columnas_a_rellenar):
        for num_columna in columnas_a_rellenar:
            for celda in sheet.iter_rows(min_row=1, min_col=num_columna, max_col=num_columna):
                celda[0].value = dato_a_poner
