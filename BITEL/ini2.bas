Sub formateaBitel()
    ...
    If Not ws Is Nothing Then
        Dim lastRowDespues As Long
            ...
            If IsEmpty(agrupadoSheet.Range("A2").Value) Then
                Dim lastRowDespues As Long
            lastRowDespues = ws.Cells(ws.Rows.Count, "A").End(xlUp).Row

            ' Copiar desde la celda A2 hasta el final de los datos en la columna A
            ws.Range("A2:A" & lastRowDespues).Copy

            ' Pegar en la hoja "AGRUPADO" a partir de la celda A2
            agrupadoSheet.Range("A2").PasteSpecial Paste:=xlPasteValues
            Application.CutCopyMode = False
        End If
    End If
